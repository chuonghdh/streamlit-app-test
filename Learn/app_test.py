...
def handle_normal_mode(row_index, row, cols):
    ...
    for j, (col_name, col_value) in enumerate(row.items()):
        if col_name == 'Image':
            ...
            if col_name in ['TestDescription', 'CreatedBy', 'LastUpdatedBy']:
    ...
    rename_button = cols[len(row)].button(
        label="✏️", 
        help="Rename",
        key=f'rename_{row_index}'
    )
    delete_button = cols[len(row) + 1].button(
        label="🗑️", 
        help="Delete",
        key=f'delete_{row_index}'
    )
    edit_questions_button = cols[len(row) + 2].button(
        label="📝", 
        help="Edit Questions",
    ...
    if edit_questions_button:
        ...
        st.session_state.page = 'edit_questions'
        st.rerun()
...
def show_page():
    ...
    if st.session_state.page == 'table':
        ...
        add_test_form()
    elif st.session_state.page == 'edit_questions':
        edit_questions_page()
